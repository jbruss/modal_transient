...
namespace ModalAnalysis
{
	...
	public:
		RungeKutta (const unsigned int n_time_steps, const double dt,
								const double wn, const double force_scale);
		void
		update_step (const double prev_disp, const double prev_veloc,
									const double gamma, double &current_disp,
									double &current_veloc);
	};

	RungeKutta::RungeKutta (const unsigned int n_time_steps, const double dt,
													const double wn, const double force_scale) :
			num_time_steps (n_time_steps), time_increment (dt), force_scale_factor (
					force_scale), wn (wn)
	{
	...
	void
	RungeKutta::update_step (const double prev_disp, const double prev_veloc,
														const double gamma, double &current_disp,
														double &current_veloc)
	{
		...
		k1v = time_increment
				* (ft - 2 * gamma * wn * prev_veloc - (wn * wn) * prev_disp);
		...
		k4v = time_increment
				* (ft_dt - 2 * gamma * wn * (prev_veloc + k3v)
						- (wn * wn) * (prev_disp + k3d));
	...
	public:
		NewmarkBetaIntegrator (const unsigned int num_steps, const double time_step,
														const double wn, const double force_scale, const double damp);
		void
	...
	{
		new_disp = (input_accel_values[step] + a1 * prev_disp + a2 * prev_veloc
				+ a3 * prev_accel) / (pow (wn, 2) + a1);
		new_veloc = (gamma / (beta * time_step)) * (new_disp - prev_disp)
				+ (1 - gamma / beta) * prev_veloc
				+ time_step * (1 - gamma / (2 * beta)) * prev_accel;
		new_accel = 1 / (beta * pow (time_step, 2)) * (new_disp - prev_disp)
				- (1 / (beta * time_step)) * prev_veloc
				- (1 / (2 * beta) - 1) * prev_accel;
		prev_disp = new_disp;
	...
	template<int dim>
		...
		private:
			...
			read_Eigen_Restart ();
			void
			get_Modal_Acceleration (const unsigned int num_time_steps,
															const double dt,
															const double wn,
															const double gamma,
															const double force_scale,
			...
			double shift;
			PETScWrappers::MPI::Vector force_vector, static_solution;
	...
	template<int dim>
				...
				computing_timer (mpi_communicator, pcout, TimerOutput::summary, TimerOutput::wall_times)
		{
			...
			parameters.declare_entry ("Boundary to Fix", "1",
																Patterns::Integer (1, 1e6),
																"The sideset to fix. (boundary_id)");
			parameters.declare_entry (
					"Run Static Solution", "false", Patterns::Bool (),
					"Run static solution if boundary is fixed and this is true.");
			parameters.declare_entry (
					"Static Force Boundary", "2", Patterns::Integer (1, 1e6),
					"The sideset to apply the force to. (boundary_id)");
			parameters.declare_entry (
					"Static Force Magnitude", "1.0", Patterns::Double (1.0, 1.0e7),
					"First modal frequency guess for the shift in the eigen-solver.");
			parameters.declare_entry (
					"Strain Energy Material ID", "2", Patterns::Integer (0, 1000),
	...
	template<int dim>
		...
		EigenvalueProblem<dim>::make_grid_and_dofs ()
		{
			...
			eigenvectors.resize (
					parameters.get_integer ("Number of eigenvalues/eigenvectors"));
			...
			DoFTools::make_sparsity_pattern (dof_handler, dsp, constraints, /*keep constrained dofs*/
																				false);
			SparsityTools::distribute_sparsity_pattern (dsp, local_dofs_per_process,
																									mpi_communicator,
																									locally_relevant_dofs);
			...
			mass_matrix.reinit (locally_owned_dofs, locally_owned_dofs, dsp,
													mpi_communicator);
			dynamic_matrix.reinit (locally_owned_dofs, locally_owned_dofs, dsp,
															mpi_communicator);
			strain_energy_matrix.reinit (locally_owned_dofs, locally_owned_dofs, dsp,
																		mpi_communicator);
...
// Assemble the global matrices
	template<int dim>
		...
		EigenvalueProblem<dim>::assemble_system ()
		{
				...
				if (cell->is_locally_owned ())
				{
					...
					for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
						for (unsigned int i = 0; i < dofs_per_cell; ++i)
						{
							...
							const double phi_i_div = fe_values[displacements].divergence (
									i, q_point);
							...
							for (unsigned int j = 0; j < dofs_per_cell; ++j)
							{
								...
								const double phi_j_div = fe_values[displacements].divergence (
										j, q_point);

								cell_stiffness_matrix (i, j) += (phi_i_div * phi_j_div
										* lambda_value
										+ 2 * (phi_i_symmgrad * phi_j_symmgrad) * mu_value)
										* fe_values.JxW (q_point);

								cell_mass_matrix (i, j) += (fe_values[displacements].value (
										i, q_point) * fe_values[displacements].value (j, q_point)
										* density_value) * fe_values.JxW (q_point);
							}
						}
					...
					constraints.distribute_local_to_global (cell_dynamic_matrix,
																									local_dof_indices,
																									dynamic_matrix);
			...
			std::pair<PETScWrappers::MatrixBase::size_type,
					PETScWrappers::MatrixBase::size_type> range;
			...
			for (unsigned int i = lo_idx; i < hi_idx; ++i)
			{
				if (constraints.is_constrained (i))
				{
					...
					{
						std::cout
								<< "One constrained DOF had mass matrix diagonal value < 1e-14. Skipping it."
								<< std::endl;
						continue;
					}
			...
			min_spurious_eigenvalue = Utilities::MPI::min (min_spurious_eigenvalue,
																											mpi_communicator);
			max_spurious_eigenvalue = Utilities::MPI::max (max_spurious_eigenvalue,
																											mpi_communicator);
			pcout << "   Spurious frequencies (Hz) are all in the interval " << "["
					<< sqrt (std::abs (min_spurious_eigenvalue + shift)) / (2 * PI)
					<< ", "
					<< sqrt (std::abs (max_spurious_eigenvalue + shift)) / (2 * PI) << "]"
					<< std::endl;
		}
	...
	template<int dim>
		...
		EigenvalueProblem<dim>::assemble_force_vector ()
		{
			...
			DoFTools::extract_boundary_dofs (dof_handler, component_mask,
																				dofs_to_apply_force, bndry_id_set);

			double static_force = parameters.get_double ("Static Force Magnitude");
			const double dof_force_value = -1.0 * static_force
					/ dofs_to_apply_force.n_elements ();
			for (auto current_idx = dofs_to_apply_force.begin ();
					current_idx != dofs_to_apply_force.end (); ++current_idx)
			{
				constraints.distribute_local_to_global (*current_idx, dof_force_value,
																								force_vector);
			}
		}
	...
	template<int dim>
		...
		EigenvalueProblem<dim>::solve_static ()
		{
			...
			SolverControl linear_solver_control (dof_handler.n_dofs (), 1.0e-12,
																						false, false);
			...
			linear_solver.solve (stiffness_matrix, static_solution, force_vector,
														preconditioner);
	...
	template<int dim>
		...
		EigenvalueProblem<dim>::solve_eigen ()
		{
			...
			pcout << "   Number of eigenpairs requested: " << eigenvalues.size ()
					<< std::endl;
			...
			if (mass_normalize)
			{
				pcout << "   Normalizing eigenvectors with respect to the mass matrix."
						<< std::endl;
				for (unsigned int i = 0; i < eigenvectors.size (); ++i)
					eigenvectors[i] /= sqrt (
							mass_matrix.matrix_scalar_product (eigenvectors[i],
																									eigenvectors[i]));
			}
			else
			{
	...
	template<int dim>
		...
		EigenvalueProblem<dim>::output_results () const
		{
			...
			if (perform_static && fix_boundary)
			{
				...
				data_out.add_data_vector (static_sol, "static_solution",
			...
			const std::string filename = ("output-"
					+ Utilities::int_to_string (this_mpi_process, 1) + ".vtu");
			...
			if (this_mpi_process == 0)
			{
				...
				for (unsigned int i = 0; i < n_mpi_processes; ++i)
					filenames.push_back (
							"output-" + Utilities::int_to_string (i, 1) + ".vtu");
	...
	template<int dim>
		...
		EigenvalueProblem<dim>::write_Eigen_Restart ()
		{
			...
			for (unsigned int i = 0; i < eigenvectors.size (); ++i)
			{
				std::string filename = ("Mode" + Utilities::int_to_string (i + 1, 2)
						+ "_" + Utilities::int_to_string (this_mpi_process, 1) + ".dat");
	...
	template<int dim>
		...
		EigenvalueProblem<dim>::read_Eigen_Restart ()
		{
			...
			std::ifstream values_input ("eigenvalues.dat", std::ifstream::in);
			double eigenval;
			for (unsigned int i = 0; i < eigenvectors.size (); ++i)
			{
				std::string filename = ("Mode" + Utilities::int_to_string (i + 1, 2)
				...
				// All of this just to get the starting DoF and ending DoF that this processor owns
				std::string str;
				input >> str;
				input >> str;
				input >> str;
				...
				std::string start_string = str.substr (0, pos1);
				std::string end_string = str.substr (pos1 + 1, pos2 - 1);
				unsigned int start = std::stoul (start_string, nullptr, 0);
				unsigned int end = std::stoul (end_string, nullptr, 0);
				double current_value;
				unsigned int dof = start;
				while ((!input.eof ()) && (dof != end + 1))
				{
					input >> current_value;
					eigenvectors[i] (dof) = current_value;
					++dof;
				}
				input.close ();

				values_input >> eigenval;
	...
	template<int dim>
																										...
																										const unsigned int local_mode_number)
		{
			...
			pcout << "   The calculated force scale factor is: " << force_scale
					<< std::endl;

			//Vector<double> displacements (num_time_steps);
			//Vector<double> velocities (num_time_steps);
			//Vector<double> accelerations (num_time_steps);
			...
			for (unsigned int step = 1; step < num_time_steps; ++step)
			{
				...
				//displacements[step] = current_disp;
				//velocities[step] = current_veloc;
				//accelerations[step] = current_accel;
			}
		}
	...
	template<int dim>
		...
		EigenvalueProblem<dim>::run ()
		{
			...
			if (write_restart)
			{
				...
				pcout << "   Eigen Solver converged in " << n_iterations
						<< " iterations." << std::endl;
			...
			{
				pcout << "   Modal Frequency " << i + 1 << " : "
						<< sqrt (std::abs (eigenvalues[i] + shift)) / (2 * PI) << " Hz"
						<< std::endl;
			}
			pcout << std::endl;
			...
			if (perform_static && fix_boundary)
			{
				...
				pcout << "   Static Solver converged in " << n_iterations
						<< " iterations." << std::endl;
			}

			// Finally we output the results to a file
			...
			for (unsigned int local_mode_number = 0; local_mode_number < num_modes_locally_owned;
					++local_mode_number)
			{
				wn = sqrt(eigenvalues[first_mode_locally_owned + local_mode_number] + shift);
